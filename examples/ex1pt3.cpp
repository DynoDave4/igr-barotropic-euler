...
int main(int argc, char *argv[])
{
   ...
   std::vector<Vector> grad_sum(nv, Vector(dim));
   std::vector<int>    grad_count(nv, 0);
   ...
   for (int e = 0; e < mesh.GetNE(); e++)
   {
      ...
      for (int lv = 0; lv < v_indices.Size(); lv++)
      {
         ...
         Vector g(dim);
         grad_x.Eval(g, *trans, ip_ref); // Evaluate ∇x at this vertex in physical coords
   ...
   for (int v = 0; v < nv; v++)
   {
      ...
      cout << ") -> grad = (";
      for (int d = 0; d < dim; d++)
         cout << grad_sum[v](d) / grad_count[v] << (d+1 < dim ? ", " : "");
      cout << ")\n";
   }
