...
void identity_map(const mfem::Vector &x, mfem::Vector &y){ y = x; }
...
class LambdaDivPart : public VectorCoefficient
{
   private:
	  ...
	  float alpha;
   public:
      LambdaDivPart(GridFunction &phi_, float alpha_) : VectorCoefficient(phi_.FESpace()->GetVDim()), phi(phi_), alpha(alpha_) {}
...
int main(int argc, char *argv[])
{
   ...
   GridFunction PhiDot(&feVECspace);
   //VectorFunctionCoefficient PhiDot_coeff(dim, identity_map);  
   ...
   VectorFunctionCoefficient Phi_coeff(dim, identity_map);   
   Phi.ProjectCoefficient(identity);                        //This block just makes a text Phi coefficient 
   ...
   LambdaDivPart DivDPhiInvT(Phi, 1.0);
