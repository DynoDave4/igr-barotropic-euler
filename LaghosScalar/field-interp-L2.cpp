...
int main (int argc, char *argv[])
{
   ...
   const char *fine_soln_file = "must_be_provided_by_the_user.gf";
   const char *prefix = "";
   ...
   args.AddOption(&src_sltn_file, "-s1", "--solution1", "GridFunction 1");
   args.AddOption(&fine_soln_file, "-s2", "--solution2", "GridFunction 2");
   ...
   {      MFEM_ASSERT(1==2, "No file given");   }

   // Input meshes.
   Mesh mesh_1(src_mesh_file, 1, 1, false);  // Coarse
   Mesh mesh_2(tar_mesh_file, 1, 1, false);  // Fine
   ...
   ifstream mat_stream_2(fine_soln_file);
   GridFunction fine_soln(&mesh_2, mat_stream_2);

   //GPT
   FiniteElementSpace *src_fes = func_source->FESpace();
   const FiniteElementCollection *fec_in = src_fes->FEColl();
   int src_vdim = src_fes->GetVDim();
   int order = src_fes->GetElementOrder(0);
   int fieldtype;
   ...
   else { MFEM_ABORT("Unsupported FE space"); }
   ...
   if (fieldtype > 1) { tar_vdim = (fieldtype == 0 || fieldtype == 1) ? dim : 1; }
   if (fieldtype == 0)
   {
      tar_fec = new H1_FECollection(order, dim);
      tar_vdim = (fieldtype > 1) ? dim : src_vdim;
   }
   else if (fieldtype == 1)
   {
   ...
   else if (fieldtype == 2)
   {
      ...
      tar_vdim = 1;
      MFEM_VERIFY(fieldtype > 1, "Cannot interpolate a scalar "
   ...
   GridFunction diff(fine_soln);
   diff -= func_target;
   ConstantCoefficient zero(0.0);
   double l2 = diff.ComputeL2Error(zero);
   std::cout << "L2 difference: " << l2 << std::endl;
