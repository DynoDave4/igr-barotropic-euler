...
void DensityIntegratorIGR::AssembleRHSElementVect(const FiniteElement &fe,
...
void ForceIntegratorIGR::AssembleElementMatrix2(const FiniteElement &trial_fe,
...
MassPAOperatorIGR::MassPAOperatorIGR(ParFiniteElementSpace &pfes,
...
void MassPAOperatorIGR::SetEssentialTrueDofs(Array<int> &dofs)
{
...
void MassPAOperatorIGR::EliminateRHS(Vector &b) const
{
   if (ess_tdofs_count > 0) { b.SetSubVector(ess_tdofs, 0.0); }
}

void MassPAOperatorIGR::Mult(const Vector &x, Vector &y) const
{
   mass->Mult(x, y);
   if (ess_tdofs_count > 0) { y.SetSubVector(ess_tdofs, 0.0); }
}

ForcePAOperatorIGR::ForcePAOperatorIGR(const QuadratureDataIGR &qdata,
...
void ForcePAOperatorIGR::Mult(const Vector &x, Vector &y) const
{
   if (L2R) { L2R->Mult(x, X); }
   else { X = x; }
...
void ForcePAOperatorIGR::MultTranspose(const Vector &x, Vector &y) const
{
   H1R->Mult(x, Y);
