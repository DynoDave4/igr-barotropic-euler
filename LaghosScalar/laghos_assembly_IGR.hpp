...
#ifndef MFEM_LAGHOS_ASSEMBLY_IGR
#define MFEM_LAGHOS_ASSEMBLY_IGR
...
struct QuadratureDataIGR
{
   ...
   // New IGR pressure term
   Vector igr_p;

   QuadratureDataIGR(int dim, int NE, int quads_per_el)
      : Jac0inv(dim, dim, NE * quads_per_el),
        stressJinvT(NE * quads_per_el, dim, dim),
        rho0DetJ0w(NE * quads_per_el),
        igr_p(NE * quads_per_el) { }
};

// This class is used only for visualization. It assembles (rho, phi) in each
// zone, which is used by LagrangianHydroOperator::ComputeDensity to do an L2
// projection of the density.
class DensityIntegratorIGR : public LinearFormIntegrator
{
   using LinearFormIntegrator::AssembleRHSElementVect;
private:
   const QuadratureDataIGR &qdata;

public:
   DensityIntegratorIGR(QuadratureDataIGR &qdata) : qdata(qdata) { }
...
class ForceIntegratorIGR : public BilinearFormIntegrator
{
private:
   const QuadratureDataIGR &qdata;
public:
   ForceIntegratorIGR(QuadratureDataIGR &qdata) : qdata(qdata) { }
...
class ForcePAOperatorIGR : public Operator
{
private:
   const int dim, NE;
   const QuadratureDataIGR &qdata;
   ...
   mutable Vector X, Y;
public:
   ForcePAOperatorIGR(const QuadratureDataIGR&,
...
class MassPAOperatorIGR : public Operator
{
private:
   const MPI_Comm comm;
   ...
   OperatorPtr mass;
public:
   MassPAOperatorIGR(ParFiniteElementSpace&, const IntegrationRule&, Coefficient&);
   virtual void Mult(const Vector&, Vector&) const;
